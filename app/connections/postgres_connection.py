...
# %% gas_analyzers
    ...
    async def get_gas_analyzer_by_measurement(self, measurement: str) -> str:
        """
        Достает из базы данных газоанализатор по заданному measurement
        :param measurement:
        :return: str
        """
        try:
            with self.connection.cursor() as cur:
                cur.execute(
                    "SELECT array_to_json(array_agg(row_to_json (r))) FROM "
                    "(SELECT * FROM Gas_analyzer WHERE measurement = %s) r;", (measurement,)
                )
                return json.dumps(cur.fetchone()[0][0], ensure_ascii=False)
        except Exception as _ex:
            return json.dumps(self.assemble_message(20, _ex), ensure_ascii=False)

    async def delete_gas_analyzer_by_measurement(self, measurement: str) -> str:
        """
        Удаляет из базы данных газоанализатор по заданному measurement
        :param measurement:
        :return:
        """
        try:
...
# %% pipes
    ...
    async def get_pipe_by_measurement(self, measurement: str) -> str:
        """
        Достает из базы данных трубу по заданному measurement
        :param measurement:
        :return: str
        """
        try:
            with self.connection.cursor() as cur:
                cur.execute(
                    "SELECT array_to_json(array_agg(row_to_json (r))) FROM "
                    "(SELECT * FROM Pipe WHERE measurement = %s) r;", (measurement,)
                )
                return json.dumps(cur.fetchone()[0][0], ensure_ascii=False)
        except Exception as _ex:
            return json.dumps(self.assemble_message(30, _ex), ensure_ascii=False)

    async def delete_pipe_by_measurement(self, measurement: str) -> str:
        """
        Удаляет из базы данных трубу по заданному measurement
        :param measurement:
        :return: str
        """
        try:
            with self.connection.cursor() as cur:
                cur.execute(
                    "DELETE FROM Pipe WHERE measurement = %s", (measurement,)
                )
                if cur.rowcount == 1:
